...
var Graph, MongoClient, Record, User, app, bodyParser, clientIdsByServerId, clientsIdsForUserId, commands, connection, env, executeCommand, express, graph, graphDef, groupClientIdsByPort, mongoDb, mysql, name, parse, processLogsCol, r, rel, rels, request, serverId, serverProcessId, snapshot, start, table, userId, userIdForClientId, userIdsByClientId, winston, _, _base, _base1, _base2, _i, _len,
...
User = (function() {
  ...
  User.prototype.logCommand = function(type, params) {
    var timestamp;
    if (env.log) {
  ...
  User.prototype.initialSnapshot = function(cb) {
    if (env.log) {
      if (!this.snapshotTaken) {
        console.log('initial snapshot');
        return request({
          method: 'get',
          url: "http://" + (env.getUpdateServer()) + "/saveSnapshot.php?userId=" + this.id + "&id=" + serverProcessId
        }, (function(_this) {
          return function(error, response, body) {
            console.log('snaphot taken', error, body);
            cb();
            return _this.snapshotTaken = true;
          };
        })(this));
      } else {
        return cb();
...
commands = {
  ...
  update: function(user, params, sendResponse) {
    return user.initialSnapshot(function() {
  ...
  subscribe: function(user, params, sendResponse) {
    ...
    clientIdsByServerId[params.serverId][params.clientId] = true;
    return user.hasPermissions(params.clientId, 'subscribe', params.object, function(permission) {
      if (permission) {
